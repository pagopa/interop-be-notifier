...
//            FormData(params.mapValues(_.toString))
  ...
  def invoke[T](
    ...
    message: String,
    pf: (Logger, String) => PartialFunction[Throwable, Future[T]] = defaultInvocationRecovery
  )(implicit m: Manifest[T], logger: Logger): Future[T] =
    execute[T](request)
      .map { response =>
  ...
  private def defaultInvocationRecovery[T]: (Logger, String) => PartialFunction[Throwable, Future[T]] =
    (logger, message) => {
