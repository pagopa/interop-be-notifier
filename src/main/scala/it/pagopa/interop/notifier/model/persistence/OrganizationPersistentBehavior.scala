...
import it.pagopa.interop.notifier.error.NotifierErrors.{OrganizationAlreadyExists, OrganizationNotFound}
  ...
  def commandHandler(
    ...
    command match {
      case AddOrganization(organization, replyTo) =>
        ...
        client
          .map { _ =>
            commandError(replyTo, OrganizationAlreadyExists(organization.organizationId.toString))
          }
          .getOrElse {
            Effect
      ...
      case UpdateOrganization(organizationId, payload, replyTo) =>
        ...
        catalogItem
          ...
          .getOrElse {
            commandError(replyTo, OrganizationNotFound(organizationId.toString))
          }
