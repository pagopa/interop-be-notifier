...
import it.pagopa.interop.notifier.model.persistence.OrganizationAdded
import it.pagopa.interop.notifier.model.persistence.serializer.v1._

import java.io.NotSerializableException

class OrganizationAddedSerializer extends SerializerWithStringManifest {
  ...
  final val className: String = classOf[OrganizationAdded].getName

  override def toBinary(o: AnyRef): Array[Byte] = o match {
    case event: OrganizationAdded =>
  ...
  override def fromBinary(bytes: Array[Byte], manifest: String): AnyRef = manifest.split('|').toList match {
    case `className` :: `version1` :: Nil =>
      deserialize(v1.events.OrganizationAddedV1, bytes, manifest, currentVersion)
    case _                                =>
      throw new NotSerializableException(
